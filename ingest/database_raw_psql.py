...
def load_data_from_csv(filePath: str, year: int):
    ...
    with Session(bind=engine) as session: 
        with session.connection().connection as raw_connection:
            try:
                # 1. Bulk Copy using COPY
                logger.info(f"Starting bulk copy from {csv_file_path} to {table_name}...")
                ...
                with open(filePath, 'r') as file:
                    with connection.cursor() as cursor:
                        ...
                        cursor.execute(text(f"COPY {table_name} FROM STDIN WITH (FORMAT CSV, HEADER TRUE)") )
                        df = pd.read_csv(csv_file_path, sep=' ', engine='python')
                        df.to_recordset(index=False).copy_to_cursor(cursor, columns=df.columns.tolist())
...
def process_directory(dirPath: str):
    ...
    for zip in zip_files: 
        ...
        load_data_from_csv(filePath=os.path.join(os.getcwd(), csv_file), year=year)
